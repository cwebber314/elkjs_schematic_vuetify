<template>
  <v-container fluid class="page-container">
    <v-row>
      <v-col>
        <h1 class="text-h4 text-primary mb-4">Update Properties Example</h1>
      </v-col>
    </v-row>

    <v-row>
      <v-col>
        <v-btn class="ml-2" color="primary" @click="openEditDialog" :disabled="!selectedNode">
          Edit Node
        </v-btn>
        <v-btn class="ml-2" color="secondary" @click="openEditEdgeDialog" :disabled="!selectedEdge">
          Edit Edge
        </v-btn>
      </v-col>
    </v-row>
...
export default {
  ...
  computed: {
    selectedNode() {
      return this.layoutNodes.find(node => node.selected)
    },
    selectedEdge() {
      return this.layoutEdges.find(edge => edge.selected)
    }
  },
  async mounted() {
    try {
  ...
  methods: {
    async computeLayout() {
      ...
      this.layoutNodes = layout.children.map(node => {
        ...
        // Copy all attributes from original node and add layout properties
        return {
          ...originalNode,  // Spread all original attributes (id, name, node_num, kv, etc.)
          id: node.id,      // Override with string ID from ELK
          x: node.x,
          y: node.y,
          width: node.width,
          height: node.height,
      ...
      this.layoutEdges = layout.edges.map(edge => {
        ...
        // Copy all attributes from original branch and add layout properties
        return {
          ...originalBranch,  // Spread all original attributes (id, name, node1_id, node2_id, ckt, etc.)
          id: edge.id,        // Override with string ID from ELK
          points,
          labelX,
          labelY,
          selected: false
    ...
    handleNodeSave(updatedNode) {
      // Find and update the node in layoutNodes
      const nodeIndex = this.layoutNodes.findIndex(n => n.id === updatedNode.id)
      if (nodeIndex !== -1) {
        // Update the node properties while preserving reactivity
        // Spread all properties from updatedNode to preserve all attributes
        this.layoutNodes[nodeIndex] = {
          ...this.layoutNodes[nodeIndex],
          ...updatedNode
        }
      }
    ...
    openEditEdgeDialog() {
      if (this.selectedEdge) {
        this.showEditEdgeDialog = true
      }
    },
    handleEdgeSave(updatedEdge) {
      // Find and update the edge in layoutEdges
      const edgeIndex = this.layoutEdges.findIndex(e => e.id === updatedEdge.id)
      if (edgeIndex !== -1) {
        // Update the edge properties while preserving reactivity
        // Spread all properties from updatedEdge to preserve all attributes
        this.layoutEdges[edgeIndex] = {
          ...this.layoutEdges[edgeIndex],
          ...updatedEdge
        }
      }
