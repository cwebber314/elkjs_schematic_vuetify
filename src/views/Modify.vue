<template>
  <v-container fluid class="page-container">
    ...
    <v-row>
      <v-col>
        <v-btn class="ml-2" color="error" @click="removeSelected" :disabled="!hasSelection">Remove</v-btn>
        <v-btn class="ml-2" color="info" @click="redraw">Redraw</v-btn>
      </v-col>
    </v-row>
...
export default {
  ...
  computed: {
    hasSelection() {
      return this.layoutNodes.some(node => node.selected) ||
             this.layoutEdges.some(edge => edge.selected)
    }
  },
  async mounted() {
    try {
  ...
  methods: {
    async computeLayout() {
      ...
      const graph = {
        ...
        edges: this.currentNetworkData.branches.map(branch => ({
          ...
          sources: [branch.node1_id],
          targets: [branch.node2_id],
      ...
      this.layoutNodes = layout.children.map(node => {
        const originalNode = this.currentNetworkData.nodes.find(n => n.id === node.id)
      ...
      this.layoutEdges = layout.edges.map(edge => {
        const originalBranch = this.currentNetworkData.branches.find(b => b.id === edge.id)
    ...
    removeSelected() {
      // Collect IDs of selected nodes and edges
      const selectedNodeIds = new Set(
        this.layoutNodes.filter(node => node.selected).map(node => node.id)
      )
      const selectedEdgeIds = new Set(
        this.layoutEdges.filter(edge => edge.selected).map(edge => edge.id)
      )

      // Check if anything is selected
      if (selectedNodeIds.size === 0 && selectedEdgeIds.size === 0) {
        console.log('No objects selected for removal')
        return
      }

      // Remove selected nodes from layout arrays
      this.layoutNodes = this.layoutNodes.filter(node => !selectedNodeIds.has(node.id))

      // Remove selected edges and edges connected to removed nodes from layout
      this.layoutEdges = this.layoutEdges.filter(edge => {
        if (selectedEdgeIds.has(edge.id)) return false
        // Check if edge connects to a removed node (need to check the data source)
        const branch = this.currentNetworkData.branches.find(b => b.id === edge.id)
        if (branch && (selectedNodeIds.has(branch.node1_id) || selectedNodeIds.has(branch.node2_id))) {
          return false
        }
        return true
      })

      // Remove from currentNetworkData
      this.currentNetworkData.nodes = this.currentNetworkData.nodes.filter(
        node => !selectedNodeIds.has(node.id)
      )

      this.currentNetworkData.branches = this.currentNetworkData.branches.filter(
        branch => {
          if (selectedEdgeIds.has(branch.id)) return false
          if (selectedNodeIds.has(branch.node1_id) || selectedNodeIds.has(branch.node2_id)) return false
          return true
        }
      )

      console.log('Removed objects. Remaining nodes:', this.currentNetworkData.nodes.length, 'Remaining branches:', this.currentNetworkData.branches.length)
    },
    async redraw() {
      try {
        this.loading = true
